...
if (isset($canview)) { unset($canview); }
if (isset($canedit)) { unset($canedit); }
if ($admin>=$edit_lvl || acl(XAT_CAN_VIEW) || acl(XAT_CAN_EDIT)) { $canview = 1; } else { $canview = 0; }
if ($admin>=$edit_lvl || acl(XAT_CAN_EDIT)) { $canedit = 1; } else { $canedit = 0; }
if ($canedit==0) { unset($edit); }
			...
			for ($x=0;$x<$rcount;$x++) {
				...
				if ($bubu->signup_ip!="" && $bubu->signup_ip!="0.0.0.0") {
					...
					if ($admin<SHOW_IP_LEVEL) {
						if ($oadm = pg_fetch_object($radm)) { $da_admlvl = $oadm->access; } else { $da_admlvl = 0; }
...
<?
	...
	std_theme_body();
        ...
        if ($admin>0 || $id==$user_id || acl(XAT_CAN_EDIT)) { $uid_info = "<font size=-1>(" . $id . ")</font>"; } else { $uid_info = ""; }
        echo("
	...
	if (pg_numrows($rrr)==0) {
		...
		while ($ooo = pg_fetch_object($rrr)) {
			if ($ooo->channel_id==1) { $isAdmin = 1; $isAdminLvl = $ooo->access; }
			if ($ooo->access==500) { $is500 = 1; }
		}
	}
	if ($edit && $isAdmin && $isAdminLvl>=$admin && $user->id!=$user_id && $admin<1000) { unset($edit); }
...
*/
	...
	echo ("</center></td></Tr>");
          if ((int)$user->flags & 1) { echo("<tr><td colspan=2 bgcolor=#" . $cTheme->main_warnmsg . "><font color=#" . $cTheme->table_headtextcolor . " size=-1><em><b><center>~ This account is currently suspended ~</center></em></b></font></td></tr>"); }
		  ...
		  if (pg_numrows($unf2)>0) { echo "<tr><td colspan=2 bgcolor=#" . $cTheme->table_tr_enlighten . "><font color=#" . $cTheme->main_textcolor . " size=-1><em><b><center>~ This account is in NOREG (user or email) ~</center></em></b></font></td></tr>\n"; }
        ...
        if (!$edit) {
        	//if ($admin>0) { local_seclog("View '" . $user->user_name . "' (" . $user->id . ")"); }
                if (($id == $user_id) || ($admin>0 || acl(XAT_CAN_EDIT))) echo("<TR><TD><font size=-1><b>Email</b></td><td><font size=-1> " . $user->email . "</font><FONT COLOR=#" . $cTheme->main_textlight . " size=-1> (Hidden from public)</td></tr>"); // Only show for self or admin
		...
		if ($admin>=NOPURGE_USER_VIEWLEVEL) {
			...
			if ((int)$user->flags & 1) { $chkd1 = " checked"; $chkd2 = ""; } else { $chkd2 = " checked"; $chkd1 = ""; }
			...
			if ($chkd1=="") { echo "Reason: <input type=text name=suspendreason size=30 value=\"\" maxlength=255>"; }
			echo "</td></tr>";
                } else {
		...
		if ($admin >= $min_lvl) {
			...
			echo "<tr><td valign=top><font size=-1><b>E-Mail Never Reg</b></td><td valign=top>[<input type=radio $chkd0 name=enreg value=1> No]&nbsp;&nbsp;&nbsp;[<input type=radio $chkd1 name=enreg value=2> Yes]&nbsp;Reason: <input type=text name=enregreason size=30 value=\"" . str_replace("\"","&quot;",$erur) . "\" maxlength=255></td></tr>";
        		...
        		echo "<td>";
        		$cyes = ""; $cno = "";
        		if ((int)$user->flags & 0x0010) { $cno = " checked"; } else { $cyes = " checked"; }
		...
		if ($admin>=NOPURGE_USER_EDITLEVEL) {
			echo "<tr><td><b>Prevent username removal:</b></td><td>";
        		$cyes = ""; $cno = "";
        		if ((int)$user->flags & 0x0020) { $cyes = " checked"; } else { $cno = " checked"; }
		...
		if ($rip && ($isAdminLvl>0 || has_acl($user->id))) { //only if the table is present... and the user has a need for IPR entries
	        	echo("<tr><td colspan=2 bgcolor=#" . $cTheme->table_sepcolor . "><font size=-1 color=#" . $cTheme->table_septextcolor . "><em><b>Access IP restrictions</b> (ACL+)</em></td></tr>");
	                ...
	                while ($ripo = @pg_fetch_object($rip)) {
	                	$amask++;
				echo "<li> ";
				if ($ripo->allowrange2 != 0) { // IP range
					echo long2ip($ripo->allowrange1) . "<b>&nbsp;&gt;&gt;&nbsp;</b>" . long2ip($ripo->allowrange2);
				} elseif ($ripo->allowrange1 != 0) { // single IP
					echo long2ip($ripo->allowrange1);
				} elseif ($ripo->allowmask != "") { // IP / Host mask
					echo $ripo->allowmask;
				}
	                	echo "\n";
	                }
	                ...
	                if (acl(XIPR_MOD_OTHERS) || ($user_id==(int)$user->id && acl(XIPR_MOD_OWN))) {
	                	echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	                	echo "<input type=button value=\"Change\" onClick=\"location.href='ip_restrict.php?user_id=" . (int)$user->id . "';\"></form>\n";
	                }
	                echo "</font></td></tr>";
	        }
	}
	...
	if ($admin>0) {
        	...
        	if (ereg("^forgotten.password.",$user->last_updated_by)) { // forgotten passord...
	...
	if ($admin>0) {
		...
		if ($edit) {
			if ($admin>=MOD_MAXLOGINS_LEVEL) {
				...
				for ($ml=1;$ml<=MAX_MAXLOGINS;$ml++) {
					echo "<option ";
					if ($ml == ($user->maxlogins+0)) { echo "selected "; }
		...
		if ($isAdmin) {
			...
			if ($admin>=800) { // new
				...
				if ($edit && $user_id!=$user->id) {
					if ((int)$user->flags & 128) { $ps1 = "selected "; $ps2 = ""; } else { $ps1 = ""; $ps2 = "selected "; }
		...
		if ($admin>=800) { // new
			...
			if ($edit) {
				if ((int)$user->flags & 256) { $ps1 = "selected "; $ps2 = ""; } else { $ps1 = ""; $ps2 = "selected "; }
	...
	if (!$edit) {
        ...
        if (!$edit) {
                if ($user->question_id=="" || $user->question_id==0)
					...
					if ($admin>=$min_lvl) {
						if (is_locked_va($user->verificationdata)) { echo "&nbsp;<i>(locked VA)</i>"; }
					}
					echo "</font>";
                ...
                if ($user->question_id=="" || $user->question_id==0)
                        ...
                        for ($x=0;$x<=$max_question_id;$x++)
                                {
                                        echo "<option ";
                                        if ($autoselect==$x) { echo "selected "; }
                                        echo "value=$x>";
                                        if ($x==0) { echo "--- pick a question ---"; } else { echo $question_text[$x]; }
                                        echo "</option>\n";
                                }
                        echo "</select>\n";
                echo "</td></tr>";
	...
	//echo $isAdmin . " " . $isAdminLvl . "<br>";
       ...
       if (  ( ($admin>=$edit_lvl || acl(XAT_CAN_EDIT)) && ((($isAdminLvl>0 && $admin>$isAdminLvl) || $admin==1000 || $id==$user_id) || $isAdmin==0) )  ) {
...
*/
        ...
        if (!$edit && $user_id==$id && ENABLE_NOTES && ((NOTES_ADMIN_ONLY && $admin>0) || (NOTES_ADMIN_ONLY==0))) { echo "</form>\n"; }
		...
		if (pg_numrows($logs)!=0) {
			...
			if ($admin>=800) { echo "<td><font color=#" . $cTheme->table_headtextcolor . ">Action</font></td>"; }
			echo "</tr>";
			$max=pg_numrows($logs);
...
//	echo "Q: $query<br>";
	...
	if (pg_numrows($logs)!=0) {
		...
		for ($row=0;$row<$max;$row++) {
			...
			echo("<tr><td>");
			echo(cs_time($log->ts) . " [" . $log->ts . "]</td><td>");
			if ($log->event>count($user_events) || $log->event==0) { echo "(old)</td><td>"; } else { echo($type[$log->event] . "</td><td>"); }
			if ($admin<SHOW_IP_LEVEL) {
