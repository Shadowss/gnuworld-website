...
if ($button == "Save Changes") {
	...
	if ($count_spc==0) {
		...
		$tmp = explode(" ",$c_mode);
		if (!ereg("k",strtolower($tmp[0])) && !ereg("l",strtolower($tmp[0]))) {
			$t2 = 1;
			$t1 = 1;
		} else {
			$t1 = strspn($tmp[0],"+stnimlkprSTNIMLKPR")!=strlen($tmp[0]);
			if (ereg("l",strtolower($tmp[0])) && ereg("k",strtolower($tmp[0]))) {
				$t2 = 1;
			} else {
				if (ereg("l",strtolower($tmp[0]))) {
					$t2 = strspn($tmp[1],"0123456789")!=strlen($tmp[1]);
				} else if (ereg("k",strtolower($tmp[0]))) {
...
if (($access >= $level_status) || ($admin > 0)) {
	...
	if ($edit && $access>=$level_set_desc) {
		...
		if (ereg("<",$channel->description) || ereg(">",$channel->description)) {
