...
/* $Id: go_perm.php,v 1.6 2002/08/15 04:53:05 nighty Exp $ */
...
function local_headers() {
	global $cTheme;
	echo "<html><head><title>" . BOT_NAME . "@ (Turn to Permanent Application)</title>";
	std_theme_styles();
	echo "</head>\n";
	std_theme_body("../../");

	echo "<h2>" . BOT_NAME . "@ Admin (Turn to Permanent Application)</h2>\n";
}
...
if ($confirm != md5( $TS . CRC_SALT_0020 . $id )) {
	...
	local_headers();
	echo "<form name=confirmation action=go_perm.php method=get>\n";
	$zets = time();
	...
	echo "<h3>";
	echo "Are your sure you want to <font color=#" . $cTheme->main_xat_goperm . ">TURN TO PERMANENT STATE</font> the following application :\n\n</h3>\n";
	...
	echo "<br><br>\n";
	echo "Which means the <b>Temporary Manager</b> will become the permanent manager of the channel.";
	echo "<br><br>\n";
	echo "<input type=button value=\"NO, CANCEL !!\" onClick=\"history.go(-1);\">";
	echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	echo "<input type=submit value=\"YES, PERM DO IT\">\n";
